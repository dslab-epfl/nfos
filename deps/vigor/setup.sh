...
# $2: "dpdk-only" to only install dpdk,
#     or no argument fall back to "no-verify"
#     must be used with "no-verify"
...
# DPDK supports make-based build before 20.11
if [[ $DPDK_RELEASE < '20.11' ]]; then
  pushd "$BUILDDIR"
    if [ ! -f dpdk/.version ] || \
...
# Requires meson-based build for DPDK 20.11 onwards
else
  pushd "$BUILDDIR"
    if [ ! -f dpdk/.version ] || \
       [ "$(cat dpdk/.version)" != "$DPDK_RELEASE" ]; then
      ...
      # Required to build DPDK since ver 20.11
      ...
      pushd dpdk
        echo "$DPDK_RELEASE" > .version
        # Apply the Vigor patches
        # TODO: check if other patches work with 20.11
        patch -p1 < "$VNDSDIR/setup/dpdk.ice.rss_reta_programming_fast_path.patch"
  
        meson build
        pushd build
          # meson configure -Dprefix="$BUILDDIR/dpdk-install"
          meson configure -Dprefix="$BUILDDIR/dpdk-install" -Dc_args=-DRTE_LIBRTE_ICE_16BYTE_RX_DESC
          ninja
          ninja install
        popd
      ...
      # Required since DPDK 20.11
      echo "export PKG_CONFIG_PATH=/$BUILDDIR/dpdk-install/lib/x86_64-linux-gnu/pkgconfig" \
        >> "$PATHSFILE"
      . "$PATHSFILE"
    fi
  popd
