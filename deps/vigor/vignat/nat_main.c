...
// #define NUM_FLOW_PARTITIONS 256
...
// expire obsolete indexes in a partition
void nf_expire_allocated_ind(vigor_time_t now, int partition) {
  flow_manager_expire(flow_manager, now, partition);
}
...
int nf_process(uint16_t device, uint8_t* buffer, uint16_t buffer_length, vigor_time_t now,
  ...
  // get endiness correct!!!
  // int partition = rte_be_to_cpu_16(tcpudp_header->dst_port) & (uint16_t)(NUM_FLOW_PARTITIONS - 1);

  // Eventually should move this code out of user's code
  ...
  if (device == config.wan_device) {
    ...
    if (!flow_manager_get_internal(flow_manager, &id, now, &external_port, partition)) {
      NF_DEBUG("New flow");

      if (!flow_manager_allocate_flow(flow_manager, &id, device, now,
                                      &external_port, partition)) {
