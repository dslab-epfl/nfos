...
static inline void *nf_borrow_next_chunk(void *p, size_t length) {
  assert(RTE_PER_LCORE(chunks_borrowed_num) < MAX_N_CHUNKS);
  void *chunk;
  packet_borrow_next_chunk(p, length, &chunk);
  RTE_PER_LCORE(chunks_borrowed)[RTE_PER_LCORE(chunks_borrowed_num)] = chunk;
  RTE_PER_LCORE(chunks_borrowed_num)++;
  return chunk;
}
...
static inline void nf_return_all_chunks(void *p) {
  while (RTE_PER_LCORE(chunks_borrowed_num) != 0) {
    packet_return_chunk(p, 
      RTE_PER_LCORE(chunks_borrowed)[RTE_PER_LCORE(chunks_borrowed_num) - 1]);
    RTE_PER_LCORE(chunks_borrowed_num)--;
  }
}
