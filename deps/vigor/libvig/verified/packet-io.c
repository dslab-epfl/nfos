...
RTE_DEFINE_PER_LCORE(size_t, global_read_length) = 0;
            ...
            *len |-> length(unread); @*/
{
  ...
  // IGNORE(p);
  RTE_PER_LCORE(global_total_length) = *len;
            ...
            chars(ptr, length, take(length, unread)); @*/
{
  ...
  *chunk = (char *)p + RTE_PER_LCORE(global_read_length);
  //@ chars_split(*chunk, length);
  RTE_PER_LCORE(global_read_length) += length;
  //@ assert *chunk |-> ?ptr;
...
/*@ ensures packetp(p, append(chnk, unread), mc); @*/
{
  ...
  RTE_PER_LCORE(global_read_length) = (uint32_t)((int8_t *)chunk - (int8_t *)p);
...
{
  //@ open packetp(p, unread, mc);
  return RTE_PER_LCORE(global_total_length) - RTE_PER_LCORE(global_read_length);
